...
$checkPhoneQuery = "SELECT COUNT(*) FROM customers WHERE phone = '$phone'";
$result = $connect->query($checkPhoneQuery);

if ($result->fetch_row()[0] > 0) {
    echo "Error: Phone number already exists. Please enter a different phone number.";
    exit();
}

// Validate if Cust_Id already exists in the 'customers' table
$checkCustIdQuery = "SELECT COUNT(*) FROM customers WHERE Cust_Id = '$cust_id'";
$result = $connect->query($checkCustIdQuery);

if ($result->fetch_row()[0] > 0) {
    echo "Error: Cust_Id already exists. Please enter a different Cust_Id.";
    exit();
}
...
if ($result->fetch_row()[0] > 0) {
    echo "Error: Email already exists. Please enter a different email.";
    exit();
}
                            ...
                            <div class="modal-body">
                                ...
                                <form action="" method="post" onsubmit="return validateForm()">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="User_Id" name="User_Id" readonly value="<?php echo $_SESSION['User_Id']; ?>">
                                    </div>
