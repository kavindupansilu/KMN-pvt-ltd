...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password && $name && $phone && $email) {
        ...
        $email = mysqli_real_escape_string($connect, $email);
            ...
            $insert_manager_query = "INSERT INTO manager (Password, Name, Phone, Email) 
            VALUES ('$password', '$name', '$phone', '$email')";


        if (mysqli_query($connect, $insert_manager_query)) {
            // Get the last inserted ID
            $last_id = mysqli_insert_id($connect);

            // Generate User_Id
            $user_id = "Manager_" . $last_id. "_ID" ;

            // Update User_Id in the database
            $update_muser_id_query = "UPDATE manager SET User_Id ='$user_id' WHERE id = $last_id";
            mysqli_query($connect, $update_muser_id_query);

            $registration_success_message = "Manager registered successfully with User_Id: $user_id";
        } else {
            echo "Error: " . $insert_manager_query . "<br>" . mysqli_error($connect);
        }
...
<div class="container">
    ...
    <form action="" method="post">
        ...
        <button type="submit" name="register_btn" class="btn btn-primary">Register</button>
            <a href="manager_login.php" class="btn btn-warning">Go to Login</a>
    </form>
